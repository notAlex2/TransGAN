...
def compute_gradient_penalty(D, real_samples, fake_samples, phi):
    ...
    fake = torch.ones([real_samples.shape[0], 1], requires_grad=False)  # .to("cpu")
    ...
    # TODO: try removing reshape, contiguous()
...
def train(
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if gen_net.module.alpha < 1:
            ...
            gen_net.module.alpha = min(1.0, gen_net.module.alpha)
        ...
        if args.loss == "hinge":
            ...
            real_label = torch.full(
                (imgs.shape[0],), 1.0, dtype=torch.float, device=real_imgs.get_device()
            )
            fake_label = torch.full(
                (imgs.shape[0],), 0.0, dtype=torch.float, device=real_imgs.get_device()
            )
            ...
            gradient_penalty = compute_gradient_penalty(
                dis_net, real_imgs, fake_imgs.detach(), args.phi
            )
            d_loss = (
        ...
        if gen_step and iter_idx % args.print_freq == 0:
            ...
            save_image(
                sample_imgs,
                f"sampled_images_{args.exp_name}.jpg",
                nrow=5,
                normalize=True,
                scale_each=True,
            )
